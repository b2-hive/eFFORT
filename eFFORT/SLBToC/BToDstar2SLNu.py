...
class BToDstar2SLNu:
    ...
    def dGamma_dw(self, w):
        ...
        return self.G_F**2 * self.eta_EW ** 2 * self.V_cb**2 * m_B**5 / (48* np.pi**3) * r**3 * (1-r)**2 * np.sqrt(w**2-1) *(w+1)**2 * (1 + 4*w/(w+1) * (1-2*r*w+r**2)/(1-r)**2)  * self.G(w)**2
...
class BToDstar2SLNuBLT(BToDstar2SLNu):
    ...
    def __init__(self, m_B: float, m_Dstar2S: float, V_cb: float, eta_EW: float = 1.0066, beta_coeff=(0.10, 2.4, -7.0)):
